...
function setUpDatabaseCli(argv: yargs.Argv) {
  return argv.command(
    'database',
    'generate PostgreSQL database docker image template',
    (dbArgs: yargs.Argv<any>): yargs.Argv<any> =>
      dbArgs.positional('name', {
        type: 'string',
        describe: 'the name of the projects',
      }),
  );
